...
def GetStudentsByClassId(class_id, option="None"):
    sql1 = "SELECT STUDENT_ID, NAME, KANA, CLASS_ID, COURSE_ID, ADDRESS, SUBSTITUTE_DAY, MEMO, LAST_UPDATE, DELETE_FLAG FROM STUDENT WHERE CLASS_ID = %s AND DELETE_FLAG = 0;"
    ...
    for i in range(len(data)):
        ...
        studentData = {
            ...
            "address": data[i][5],
            "substitute_day": str(data[i][6]),
            "memo": data[i][7],
            "last_update": data[i][8].strftime("%Y-%m-%d"),
